...
// importing routes
const courseRouter = require('./adminRoutes/courseRoutes');
const quizzRouter = require('./adminRoutes/quizz');
				...
				bcrypt.genSalt(10, (err, salt) => {
					bcrypt.hash(newAdmin.password, salt, (err, hash) => {
						...
						newAdmin
							...
							.catch(err => console.log('error while saving the admin to database' + err));
					});
				});
			}
		})
...
router.post('/login', (req, res) => {
	...
	Admin.findOne({ email })
		.then(admin => {
			if (!admin) {
				return res.status(404).json({ emailErr: 'Admin not found with this email' });
			}
...
router.use('/course', passport.authenticate('admin', { session: false }), courseRouter);
router.use('/quizz', passport.authenticate('admin', { session: false }), quizzRouter);
