...
export default class MyCourses extends React.Component {
  ...
  _getWebCourses = async () => {
    console.log("State token" + this.state.token);
    let headers = {
      "Content-Type": "application/json",
      Authorization: this.state.token
    };
    await Axios.get(`http://${ip.default}:5000/user/course/web`, {
      headers: headers
    })
      .then(res => {
      ...
      .catch(err =>
        console.log("Error while getting web courses route " + err)
      );
  };
      ...
      <Container style={styles.container}>
        <Content>
          <View style={styles.componentTop}>
            <Text style={styles.componentHeading}>Web Development</Text>
