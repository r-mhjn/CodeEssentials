...
export default class MyCourses extends React.Component {
	...
	async componentDidMount() {
		...
		this._getDataScienceCourses();
	}

	_getDataScienceCourses = async () => {
		// console.log("State token" + this.state.token);
		let headers = {
			'Content-Type': 'application/json',
			Authorization: this.state.token,
		};
		await Axios.get(`http://${ip.default}:5000/user/course/datascience`, {
			headers: headers,
		})
			.then(res => {
			...
			.catch(err => console.log('Error while getting datascience courses route ' + err));
	};
